// derived from
// https://github.com/denoland/deno/blob/main/runtime/inspector_server.rs
...
impl InspectorServer {
    pub fn new(inspector_port: u16) -> Result<Self, Error> {
...
fn handle_ws_request(
    ...
    spawn(async move {
        ...
        println!("Debugger session started.");
...
async fn server(
    ...
    let listener = match TcpListener::from_std(listener) {
        Ok(l) => l,
        Err(err) => {
            println!("Cannot start inspector server: {:?}", err);
            return;
        }
    };
